...
class Customer {
	...
	public function getRewardPoints($data = array()) { 
		...
		if (!empty($data['filter_date_start'])) {
			$sql .= "DATE(cr.date_added) >= " . $this->db->quote($data['filter_date_start']);
		}

		if (!empty($data['filter_date_end'])) {
			$sql .= "DATE(cr.date_added) <= " . $this->db->quote($data['filter_date_end']);
		}
	...
	public function getTotalRewardPoints() {
		...
		if (!empty($data['filter_date_start'])) {
			$implode[] = "DATE(cr.date_added) >= " . $this->db->quote($data['filter_date_start']);
		}

		if (!empty($data['filter_date_end'])) {
			$implode[] = "DATE(cr.date_added) <= " . $this->db->quote($data['filter_date_end']);
		}
	...
	public function getCredit($data = array()) { 
		...
		if (!empty($data['filter_date_start'])) {
			$sql .= "DATE(ct.date_added) >= " . $this->db->quote($data['filter_date_start']);
		}

		if (!empty($data['filter_date_end'])) {
			$sql .= "DATE(ct.date_added) <= " . $this->db->quote($data['filter_date_end']);
		}
