...
class Voucher_theme {
	...
	public function addVoucherTheme($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "voucher_theme SET image = " . $this->db->quote(html_entity_decode($data['image'], ENT_QUOTES, 'UTF-8')));
		
		$voucher_theme_id = $this->db->getLastId();
		
		foreach ($data['voucher_theme_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "voucher_theme_description SET voucher_theme_id = '" . (int)$voucher_theme_id . "', language_id = '" . (int)$language_id . "', name = " . $this->db->quote($value['name']));
		}
	...
	public function editVoucherTheme($voucher_theme_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "voucher_theme SET image = " . $this->db->quote(html_entity_decode($data['image'], ENT_QUOTES, 'UTF-8')) . " WHERE voucher_theme_id = '" . (int)$voucher_theme_id . "'");
