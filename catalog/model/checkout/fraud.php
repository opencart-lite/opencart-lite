		...
		if ($fraud_info) {
			...
			*/		
			...
			if ($response) {
				...
				$this->db->query("INSERT INTO `" . DB_PREFIX . "order_fraud` SET order_id = '" . (int)$order_id . "', customer_id = '" . (int)$customer_id . "', country_match = " . $this->db->quote($country_match) . ", country_code = " . $this->db->quote($country_code) . ", high_risk_country = " . $this->db->quote($high_risk_country) . ", distance = '" . (int)$distance . "', ip_region = " . $this->db->quote($ip_region) . ", ip_city = " . $this->db->quote($ip_city) . ", ip_latitude = " . $this->db->quote($ip_latitude) . ", ip_longitude = " . $this->db->quote($ip_longitude) . ", ip_isp = " . $this->db->quote($ip_isp) . ", ip_org = " . $this->db->quote($ip_org) . ", ip_asnum = " . (int)$ip_asnum . ", ip_user_type = " . $this->db->quote($ip_user_type) . ", ip_country_confidence = " . $this->db->quote($ip_country_confidence) . ", ip_region_confidence = " . $this->db->quote($ip_region_confidence) . ", ip_city_confidence = " . $this->db->quote($ip_city_confidence) . ", ip_postal_confidence = " . $this->db->quote($ip_postal_confidence) . ", ip_postal_code = " . $this->db->quote($ip_postal_code) . ", ip_accuracy_radius = '" . (int)$ip_accuracy_radius . "', ip_net_speed_cell = " . $this->db->quote($ip_net_speed_cell) . ", ip_metro_code = '" . (int)$ip_metro_code . "', ip_area_code = '" . (int)$ip_area_code . "', ip_time_zone = " . $this->db->quote($ip_time_zone) . ", ip_region_name = " . $this->db->quote($ip_region_name) . ", ip_domain = " . $this->db->quote($ip_domain) . ", ip_country_name = " . $this->db->quote($ip_country_name) . ", ip_continent_code = " . $this->db->quote($ip_continent_code) . ", ip_corporate_proxy = " . $this->db->quote($ip_corporate_proxy) . ", anonymous_proxy = " . $this->db->quote($anonymous_proxy) . ", proxy_score = '" . (float)$proxy_score . "', is_trans_proxy = " . $this->db->quote($is_trans_proxy) . ", free_mail = " . $this->db->quote($free_mail) . ", carder_email = " . $this->db->quote($carder_email) . ", high_risk_username = " . $this->db->quote($high_risk_username) . ", high_risk_password = " . $this->db->quote($high_risk_password) . ", bin_match = " . $this->db->quote($bin_match) . ", bin_country = " . $this->db->quote($bin_country) . ",  bin_name_match = " . $this->db->quote($bin_name_match) . ", bin_name = " . $this->db->quote($bin_name) . ", bin_phone_match = " . $this->db->quote($bin_phone_match) . ", bin_phone = " . $this->db->quote($bin_phone) . ", customer_phone_in_billing_location = " . $this->db->quote($customer_phone_in_billing_location) . ", ship_forward = " . $this->db->quote($ship_forward) . ", city_postal_match = " . $this->db->quote($city_postal_match) . ", ship_city_postal_match = " . $this->db->quote($ship_city_postal_match) . ", score = '" . (float)$score . "', explanation = " . $this->db->quote($explanation) . ", risk_score = '" . (float)$risk_score . "', queries_remaining = '" . (int)$queries_remaining . "', maxmind_id = " . $this->db->quote($maxmind_id) . ", error = " . $this->db->quote($error) . ", date_added = NOW()");
			}
		}
